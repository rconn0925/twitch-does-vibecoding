...
### Donation platform & event ingestion (LOCKED by research pass — see 04-RESEARCH.md)
- **D-01 (LOCKED 2026-07-10):** **StreamElements** — cloud-only realtime websocket via `socket.io-client` against `realtime.streamelements.com`, JWT/OAuth2 `authenticate` handshake, `tip` event; no desktop client required (primary-sourced from StreamElements docs, 04-RESEARCH.md Q1). Bits as a paid-influence trigger is **rejected with verbatim AUP citation** (current policy prohibits exchanging Bits for off-Twitch experiences / goods-with-monetary-cost — a donor-directed build session is exactly that; 04-RESEARCH.md Q2). Chargeback handling: **build nothing for MVP** — SE.Pay tips are non-refundable viewer-side and windows are short; PAID-04 is satisfied by the `control_windows` amount→duration ledger (Q3). ⚠ AUP quotes + chargeback policy are WebSearch-summarized — manual re-read flagged before treating as load-bearing compliance text (part of the end human-gate batch).
- **D-02 (corrected by research):** Channel-points trigger = native Twitch EventSub `channel.channel_points_custom_reward_redemption.add` v1, scope `channel:read:redemptions`. **Phase 2's token does NOT yet carry this scope** — Phase 4 must add it to `TWITCH_SCOPES` and include a broadcaster re-authorization step (live human gate, batched at end). Single dedicated reward for MVP (multi-tier deferred). Single-broadcaster-token default unchanged.
...
### Paid-window control model & lifetime (LOCKED by user 2026-07-10)
- **D-11 (control model = OPEN during window):** While a paid free-reign window is active, **any chatter may submit build instructions** — the donor *sponsors an open, accelerated build slot* rather than getting private personal control. There is NO donor-identity gate on `submitInstruction` (resolves plan-checker W1 — no guard needed BY DESIGN). Every submission during the window STILL passes the identical compliance gate (gate.classify → single funnel) and stays streamer-vetoable (PAID-03 unchanged); the window's only effect is that gate-compliant submissions bypass the VOTE and go straight to the build queue, one at a time. **UI/narration framing:** present the window as "sponsored by {donorDisplayName}" / an open build slot — NOT "{donor} is in control." Overlay stays coarse (donorDisplayName + countdown only).
- **D-12 (lifetime = full paid duration):** A window runs its **entire amount-proportional duration** (NOT closed on first accepted build). Multiple gate-compliant builds run one-at-a-time (existing concurrency-1) for the window's lifetime; on expiry OR streamer revoke, revert to the normal vote/IDLE loop (resolves plan-checker I1). `endsAtMs` is absolute and crash-safe (restore re-arms remaining time only).
